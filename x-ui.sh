...
install_base() {
    ...
    centos | almalinux | rocky | ol)
        yum install -y -q wget curl tar tzdata
        ;;
    fedora | amzn | virtuozzo)
        dnf install -y -q wget curl tar tzdata
        ;;
    arch | manjaro | parch)
        pacman -S --noconfirm wget curl tar tzdata
        ;;
    opensuse-tumbleweed)
        zypper -q install -y wget curl tar timezone
        ;;
    *)
        apt-get install -y -q wget curl tar tzdata
        ;;
